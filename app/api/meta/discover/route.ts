...
import { metaOAuthSessionRepo } from "@/lib/server/db";
...
export async function GET(request: Request) {
  ...
  const session = metaOAuthSessionRepo.get<DiscoveredRawPage[]>(sessionId);
  if (!session) {
  ...
  const summary: DiscoverySessionSummary = {
    ...
    assets: toDiscoveredAssets(session.discovered),
  };
