...
import { metaOAuthSessionRepo, publicationLogsRepo, socialAccountsRepo } from "@/lib/server/db";
 ...
 *  seguro devolver direto pro frontend. */
export async function GET() {
  return NextResponse.json(socialAccountsRepo.list());
}
...
export async function POST(request: Request) {
  ...
  const session = metaOAuthSessionRepo.get<DiscoveredRawPage[]>(body.sessionId);
  if (!session) {
  ...
  metaOAuthSessionRepo.remove(body.sessionId);

  return NextResponse.json({ connected: connectedIds.length, skipped, accounts: socialAccountsRepo.list() });
}
