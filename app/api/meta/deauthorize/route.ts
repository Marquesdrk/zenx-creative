...
export async function POST(request: Request) {
  ...
  try {
    ...
    if (payload.user_id) {
      const all = await socialAccountsRepo.list();
      const affected = all.filter((a) => a.metaUserId === payload.user_id && a.status !== "revoked");
      for (const account of affected) {
        await socialAccountsRepo.disconnect(account.id);
