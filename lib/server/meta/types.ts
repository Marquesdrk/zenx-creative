...
export type SocialAccount = {
  ...
  userId: string | null;
  /** Reservado para quando o sistema ganhar múltiplos projetos/marcas — hoje é sempre null. */
  ...
  /** Escopos concedidos no consentimento, capturados via /debug_token (nunca o token em si). */
  permissions: string[];
...
/** Etapas do fluxo Meta logadas estruturadamente em meta_integration_logs (ver lib/server/meta/log.ts).
 *  Cada etapa é gravada com endpoint/http_status/error_code/error_subcode/fbtrace_id quando
 *  aplicável — nunca com um token. */
export type MetaIntegrationStep =
  | "META_OAUTH_STARTED"
  | "META_OAUTH_CALLBACK"
  | "META_TOKEN_RECEIVED"
  | "META_PAGES_FETCHED"
  | "META_INSTAGRAM_ACCOUNT_FOUND"
  | "META_ACCOUNT_SAVED"
  | "META_CONNECTION_VALIDATED"
  | "META_MEDIA_CONTAINER_CREATED"
  | "META_MEDIA_PROCESSING"
  | "META_MEDIA_PUBLISHED"
  | "META_API_ERROR";

export type MetaIntegrationLogEntry = {
  id: string;
