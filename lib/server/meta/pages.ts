 ...
 *  descoberta criptografada no servidor; ver metaOAuthSessionRepo em lib/server/db.ts). */
export function toDiscoveredAssets(pages: DiscoveredRawPage[]): DiscoveredAsset[] {
  ...
  for (const page of pages) {
    const fbAsset: DiscoveredFacebookPage = {
      ...
      alreadyConnected: Boolean(socialAccountsRepo.findByPlatformAccountId("FACEBOOK", page.pageId)),
    };
    assets.push(fbAsset);
    ...
    if (page.instagram) {
      const igAsset: DiscoveredInstagramAccount = {
        ...
        alreadyConnected: Boolean(socialAccountsRepo.findByPlatformAccountId("INSTAGRAM", page.instagram.id)),
      };
      assets.push(igAsset);
    }
  }
