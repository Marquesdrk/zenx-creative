 ...
 *  variável de ambiente global, cada chamada é isolada por conta. `onEvent` (opcional) emite as
 *  etapas META_MEDIA_CONTAINER_CREATED/META_MEDIA_PROCESSING pro logger estruturado do chamador. */
export async function publishInstagramReel(params: {
  ...
  params.onEvent?.("META_MEDIA_CONTAINER_CREATED", { containerId, platform: "INSTAGRAM" });
  try {
    params.onEvent?.("META_MEDIA_PROCESSING", { containerId });
